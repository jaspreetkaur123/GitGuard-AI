const { getOctokitInstance, fetchDiff, postComment } = require('../services/github.service');
  ...
  try {
    ...
    if (event === 'pull_request') {
      ...
      if (action === 'opened' || action === 'synchronize') {
        ...
        const repoDoc = await Repository.findOne({ githubRepoId: repository.id.toString() });
        if (repoDoc) {
          const limitCheck = await checkUsageLimit(repoDoc.owner);
          if (!limitCheck.allowed) {
            console.warn(`Limit reached for repo owner ${repoDoc.owner}: ${limitCheck.message}`);
            // Optionally: Post a comment to PR saying limit is reached
            const octokit = getOctokitInstance(installation.id);
            await postComment(octokit, owner, repoName, pullNumber, `🚫 **Limit Reached**: ${limitCheck.message}`);
            return res.status(200).json({ received: true, message: 'Limit reached' });
          }
        }
        ...
        await postComment(octokit, owner, repoName, pullNumber, analysisResult);

        // 5. Log the activity (Optional: Find the repo in our DB first)
        const repo_Doc = await Repository.findOne({ githubRepoId: repository.id.toString() });
        await ReviewLog.create({
          repository: repo_Doc ? repo_Doc._id : null,
